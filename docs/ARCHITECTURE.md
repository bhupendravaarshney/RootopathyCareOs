...
Browser authentication uses persisted password credentials, Redis-backed indexed sessions, Origin/Referer and CSRF checks, encrypted TOTP/recovery factors, recent-authentication evidence, throttling, and security-version revocation. The React session feature bootstraps and runtime-validates the session, locks protected content for anonymous/pending/error/no-membership states, and calls the checked client for primary login, pending MFA, organization selection/switching, and acknowledged logout without browser-persistent credential state. Organization selection is derived from live membership and stored server-side for navigation, but it is not authorization evidence. Every governed tenant operation must independently enter `TenantAuthorizationOperations`. The authorization catalogs are intentionally empty and runtime-read-only until the owner-approved policy registry is delivered, so unknown roles grant no permissions. See `FRONTEND_SESSION.md`.
...
The contract also defines the required `/api/v1/organizations/{organizationId}/...` protected-business prefix, opaque cursor and bounded-limit components, operation-specific filter allowlists, strong `ETag`/`If-Match` concurrency, scoped idempotency, and caller-controlled retry semantics. A dependency-pinned generator produces checked TypeScript artifacts, and the small browser client wraps all 15 current operations with credentials, CSRF bootstrap, correlation, strict response handling, and no automatic mutation retry. CORS permits `If-Match`/`Idempotency-Key`, exposes `ETag`/`Retry-After`, and deliberately does not advertise `X-Organization-Id`. See `API_CONVENTIONS.md`.
