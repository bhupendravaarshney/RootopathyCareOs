...
**Status:** Exact `m1-candidate-1` inputs approved unchanged; production input gate complete; checksum-bound registry promotion, M1-20 membership read/change/owner-transfer, approved mandatory-role MFA, M1-20 responsive table/card adaptation, shared M1-01 through M1-04 conformance, exact readiness projection, and exact M1-07 organization profile complete; additive facility-scope contract ready for approval; M1B and M1C implementation remain in progress
...
M1-05 through M1-07 use a bounded `administration` feature backed by protected tenant APIs. M1-05/M1-06 consume all 15 exact ordered `m1-readiness-v1` live gates, approved outcomes, database-owned 15-minute freshness, bounded evidence references, and permission-safe deep links. M1-05 renders metrics/priority exceptions while M1-06 renders the full checklist and labels the persisted-validation boundary. M1-07 implements the approved legal/display/trading identity, organization type, ISO country, IANA timezone, BCP 47 locale, exact lifecycle projection, revision, and caller editability with stable field Problems, strong ETag concurrency, caller-owned idempotency, explicit reason, and atomic sorted changed-field audit/outbox evidence. V20 promotes the operation/permission/event bindings, and V25 enforces the exact persisted profile boundary. A valid current profile now completes `organization.profile.complete`; legacy null type/locale rows remain readable and readiness-blocked until a governed update. Other missing entity-specific evaluators and persisted validation/activation remain M1C/M1F work.
...
M1-08 through M1-19 and M1-21 through M1-23 remain generic prototype templates with hard-coded synthetic records, dates, and statuses. Generic pages state the synthetic/no-real-data boundary, keep form fields read-only, disable unimplemented workflow actions, remove misleading record links, and limit interaction to honest local filtering and valid prototype pagination. The approved catalogues now exist, but most screens still have no protected business API, business persistence, operation binding, canonical event emission, or screen-specific production states.
...
| M1-08 Registration/identifiers | Registration numbers and schemes                               | Generic local-only form                                                                                                                                                                                                                                                  | Implement approved authorities, jurisdiction validation, uniqueness, history, evidence, API/UI             |
| M1-09 Addresses/contacts       | Effective contact and address records                          | Generic local-only form                                                                                                                                                                                                                                                  | Implement approved types, effective dating, primary rules, validation, sensitivity, and history            |
| M1-10 International settings   | Country, timezone, locale, date/number rules                   | Generic local-only form                                                                                                                                                                                                                                                  | Implement approved values, defaults, change impact, effective-time behavior, API/UI, and evidence          |
| M1-11 Governance contacts      | Clinical, privacy, security, billing contacts                  | Generic local-only form                                                                                                                                                                                                                                                  | Implement approved responsibilities, account/person linkage, escalation data, and visibility               |
| M1-12 Facilities               | Server list/filter and one add action                          | Generic hard-coded list                                                                                                                                                                                                                                                  | Implement approved fields, server filters/sorts, lifecycle visibility, permissions, API/UI                 |
...
The bounded `administration` feature is composed by the root without cross-feature imports. Its state is keyed by the selected organization and discarded on organization change. It drives M1-05 through M1-07 and the real M1-20 membership read/change states. M1-05 and M1-06 reject readiness catalogue/version/order/count/freshness/evidence/link drift before rendering distinct dashboard and checklist projections; M1-07 rejects malformed profile/ETag responses, supports the exact approved fields, and projects current manage permission into editable or read-only UI; M1-20 implements the approved result table/card breakpoint. The shared identity/workspace frame contract for M1-01 through M1-04 includes the approved font/content, hash-safe skip navigation, route-heading focus, and problem-summary focus precedence; local form validation adds focused summaries, exact field association, hash-safe field navigation, and two-pixel error treatment. M1-05 through M1-07 remain within incomplete M1C, while M1B retains separately authorized invitation operations plus explicit owner/target acceptance. Future pages must continue to compose the feature, and shared components must contain no administration rules.
...
Base input approval is complete. M1B has five bounded runtime increments plus shared frontend conformance, but remains open for facility-scoped membership grants, separately authorized invitation inspection/resend/delivery behavior, explicit owner acceptance, and target-environment evidence. V20-V25 deliver the approved registry, identity/access workflows, owner safety, mandatory MFA, and exact organization profile; Phases 1AH-1AJ deliver the responsive M1-20 and shared identity contracts. Phase 1AK begins M1C with the exact ordered live-readiness catalogue, and Phase 1AL completes the approved M1-07 profile fields, validation, persistence, UI, evidence, editability, and live profile evaluator. Verification is at 181 backend tests, 57 frontend unit tests, and 95 five-viewport browser/Axe cases; the frontend boundary covers 22 source files and 49 relative imports. Nine tests lock OpenAPI conventions/profile drift, and seven tests lock the non-authorizing facility-scope candidate. Neither M1B nor M1C has met its exit condition.
...
This closes the exact M1-07 vertical slice, not M1C. Registration identifiers, effective addresses/contacts, international change impact, governance responsibilities, configuration-version invalidation, immutable validation, activation, history, and export remain separate approved slices.
