...
export type EncounterResponseValidator<T> = (value: unknown) => value is T;

const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const screenPattern = /^P5-(0[1-9]|1[0-2])$/;
const actionKeyPattern = /^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/;
const fieldKeyPattern = /^[A-Za-z][A-Za-z0-9_.-]{0,79}$/;
const linkPattern = /^#\/P5-(0[1-9]|1[0-2])$/;
const fieldTypes = new Set(['checkbox', 'datetime-local', 'select', 'text', 'textarea', 'uuid']);
...
function isRow(value: unknown, screenId: string): value is EncounterRow {
  return (
    hasExactKeys(
      value,
      ['id', 'status', 'revision', 'etag', 'values', 'allowedActionKeys'],
      ['patientId', 'episodeId', 'encounterId', 'appointmentId'],
    ) &&
    isUuid(value.id) &&
    isOptionalUuid(value.patientId) &&
    isOptionalUuid(value.episodeId) &&
    isOptionalUuid(value.encounterId) &&
    isOptionalUuid(value.appointmentId) &&
    isNonEmptyString(value.status) &&
    Number.isSafeInteger(value.revision) &&
    Number(value.revision) >= 0 &&
    value.etag === `"m5:${screenId}:${value.id as string}:${value.revision as number}"` &&
...
export function encounterScreenValidator(
  organizationId: string,
  screenId: string,
): EncounterResponseValidator<EncounterScreen> {
  return (value): value is EncounterScreen => {
    if (!(
      hasExactKeys(value, [
        'organizationId',
        'screenId',
      ...
      isArrayOf(value.rows, (row): row is EncounterRow => isRow(row, screenId)) &&
    ...
    const rows = value.rows as EncounterRow[];
