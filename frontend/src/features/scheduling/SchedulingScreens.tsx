...
const requestScreens = new Set(['P4-04', 'P4-05', 'P4-06', 'P4-07', 'P4-09', 'P4-11']);
...
function emptyContext(): WorkflowContext {
  return { appointmentId: null, patientId: null, requestId: null };
}
...
function contextFromHash(): WorkflowContext {
  ...
  for (const key of ['appointmentId', 'patientId', 'requestId'] as const) {
...
function contextForRow(screenId: string, row: SchedulingRow, current: WorkflowContext) {
  return {
    appointmentId: row.appointmentId ?? current.appointmentId,
    patientId: row.patientId ?? current.patientId,
    requestId:
      (requestScreens.has(screenId) ? row.id : null) ?? row.values.requestId ?? current.requestId,
  };
}
...
function statusTone(status: string) {
  if (['active', 'available', 'booked', 'confirmed', 'complete'].includes(status)) return 'success';
  if (['cancelled', 'expired', 'failed', 'no_show', 'withdrawn'].includes(status)) return 'danger';
  if (['collecting', 'draft', 'held', 'pending', 'waiting'].includes(status)) return 'warning';
  return 'neutral';
}
...
function idempotencyKey(actionKey: string) {
  ...
  return `m4:${actionKey}:${globalThis.crypto.randomUUID()}`;
}
...
function fieldDefault(field: WorkforceField, selected: SchedulingRow | undefined) {
  if (!selected) return field.options[0]?.value ?? '';
  if (field.key === 'slotId') return selected.id;
...
function ActionDialog({
  ...
  selected: SchedulingRow | undefined;
}) {
  ...
  return (
    <div className="workforce-dialog-backdrop">
      <section
        aria-labelledby="scheduling-action-title"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <span className="eyebrow">Governed scheduling action</span>
            <h2 id="scheduling-action-title">{action.label}</h2>
          </div>
          <button
            ...
            <label className="full-width">
              ...
              <small>
                {selected
                  ...
                  : 'Select an authorized scheduling record.'}
              </small>
            </label>
          )}
...
export function SchedulingScreenPage({
  client,
  id,
  organizationId,
  shell,
  ...
  const [projection, setProjection] = useState<SchedulingScreen | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setLoadIssue(null);
      setSuccess(null);
      const result = await client.getSchedulingScreen(
        organizationId,
        id,
        {
  ...
  const previousId = moduleNumber > 1 ? `P4-${String(moduleNumber - 1).padStart(2, '0')}` : null;
  const nextId = moduleNumber < 15 ? `P4-${String(moduleNumber + 1).padStart(2, '0')}` : null;

  function applyRowContext(row: SchedulingRow) {
    const nextContext = contextForRow(id, row, context);
  ...
  async function submitAction(submission: ActionSubmission) {
    ...
    try {
      ...
      const requestId = requestScreens.has(id) && selected ? selected.id : context.requestId;
      if (activeAction.key === 'hold-slot' && !requestId) {
        throw new Error('Select or apply an appointment request before holding a slot.');
      }
      ...
      const body: SchedulingActionRequest = {
  ...
  return (
    <Shell currentId={id} {...shell}>
      ...
      <details className="panel workforce-actions-panel">
        ...
        <p>Carry an authorized request, patient or appointment through the scheduling screens.</p>
        <form
        ...
        >
          {(['requestId', 'patientId', 'appointmentId'] as const).map((key) => (
            <label key={key}>
              {humanize(key)}
              <input
                autoComplete="off"
      ...
      {loading && (
        <section aria-live="polite" className="panel workforce-state">
          ...
          <h2>Loading authorized scheduling data…</h2>
      ...
      {!loading && projection && (
        <>
          ...
          {projection.metrics.length > 0 && (
            <section aria-label="Scheduling metrics" className="workforce-metrics">
          ...
          <section className="panel workforce-directory-panel">
            <form
              aria-label="Scheduling record filters"
              className="workforce-filters"
              onSubmit={(event) => {
                event.preventDefault();
                ...
                if (query.length === 1) {
                  setLoadIssue({
                    detail: 'Scheduling searches require at least two characters.',
            ...
            <div className="workforce-table-heading">
              <div>
                <h2>Authorized scheduling records</h2>
                <p aria-live="polite">
                  {projection.rows.length} record{projection.rows.length === 1 ? '' : 's'} returned
                </p>
              </div>
              <button
                aria-label="Refresh scheduling records"
                className="icon-button"
                onClick={() => setRefresh((value) => value + 1)}
                type="button"
              >
            ...
            <div aria-label="Scheduling result pages" className="workforce-cursor-pager">
              <button
                className="secondary-button"
          ...
          <section
            aria-labelledby="scheduling-actions-title"
            className="panel workforce-actions-panel"
          >
            <div>
              <h2 id="scheduling-actions-title">Available actions</h2>
              <p>
