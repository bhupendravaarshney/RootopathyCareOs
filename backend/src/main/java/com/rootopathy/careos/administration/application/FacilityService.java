...
public final class FacilityService {
 public static final String READ="network.facility.read",MANAGE="network.facility.manage",LIFECYCLE="network.facility.lifecycle";
 ...
 public IdempotencyOutcome activate(Lifecycle c){return transition(c,"under_review","active","activated",null);}public IdempotencyOutcome suspend(Lifecycle c){return transition(c,"active","suspended","suspended",null);}public IdempotencyOutcome reactivate(Lifecycle c){return transition(c,"suspended","active","reactivated",null);}public IdempotencyOutcome close(Lifecycle c){if(!Set.of("active","suspended").contains(c.fromState))throw new IllegalArgumentException("invalid closure source state");return transition(c,c.fromState,"closed","closed",text(c.reason,"reason",10,500));}
 private IdempotencyOutcome transition(Lifecycle c,String from,String to,String change,String closureReason){var reason=text(c.reason,"reason",10,500);validKey(c.idempotencyKey);var revision=revision(c.ifMatch,c.facilityId);var command=new IdempotencyCommand(LIFECYCLE,c.idempotencyKey,hash(c.facilityId,revision,from,to,reason),clock.instant().plusSeconds(86400));return mutations.execute(request(c.organizationId,c.actorId,c.correlationId,LIFECYCLE,reason,c.recentAuthenticationAt,c.mfaAuthenticatedAt),command,ctx->{var r=store.transition(ctx,c.facilityId,revision,from,to,closureReason);var payload=json(Map.of("facilityId",r.facilityId(),"fromState",from,"toState",to,"lockVersion",r.lockVersion()));var event="facility."+change;return new GovernedMutation(new IdempotentResponse(200,"application/json",json(r.directory())),new GovernanceEvidence(new AuditRecord(event,1,"facility",r.facilityId(),reason,payload),new OutboxRecord(event,1,"facility",r.facilityId(),payload)));});}
 ...
 private TenantAuthorizationRequest request(UUID o,UUID a,String c,String op,String reason,java.time.Instant recent,java.time.Instant mfa){return new TenantAuthorizationRequest(o,new AuthenticatedActorContext(a,"facility-administration",c),new OperationKey(op),reason,recent,mfa,null);}
 ...
 public record Lifecycle(UUID organizationId,UUID actorId,String correlationId,UUID facilityId,String ifMatch,String idempotencyKey,String fromState,String reason,java.time.Instant recentAuthenticationAt,java.time.Instant mfaAuthenticatedAt){}
}
