...
@RestControllerAdvice(assignableTypes = OrganizationInternationalSettingsController.class)
public final class OrganizationInternationalSettingsProblemHandler {
    @ExceptionHandler(OrganizationInternationalSettingsException.class)
    ResponseEntity<ProblemDetail> settingsProblem(
            OrganizationInternationalSettingsException exception,
            HttpServletRequest request) {
        var mapping = switch (exception.reason()) {
            case INVALID_REQUEST -> new ProblemMapping(
                    HttpStatus.BAD_REQUEST,
                    "organization-settings-invalid",
                    "Invalid international settings");
            ...
            case PRECONDITION_REQUIRED -> new ProblemMapping(
                    ...
                    "settings-precondition-required",
                    "Settings precondition required");
            case STALE_REVISION -> new ProblemMapping(
                    HttpStatus.PRECONDITION_FAILED,
                    "settings-stale-revision",
                    "International settings changed");
            case NO_CHANGE -> new ProblemMapping(
                    HttpStatus.CONFLICT,
                    "m1.settings.no_change",
                    "No international-settings change");
            case SCHEDULE_CONFLICT -> new ProblemMapping(
                    HttpStatus.CONFLICT,
                    "m1.settings.schedule_conflict",
                    "International-settings schedule conflict");
            case EFFECTIVE_OVERLAP -> new ProblemMapping(
                    HttpStatus.CONFLICT,
                    "m1.effective.overlap",
                    "International-settings effective overlap");
            case INVALID_TRANSITION -> new ProblemMapping(
                    HttpStatus.CONFLICT,
                    "m1.lifecycle.invalid_transition",
                    "Invalid international-settings transition");
        };
