...
@RestControllerAdvice(assignableTypes = OrganizationContactController.class)
public final class OrganizationContactProblemHandler {
    @ExceptionHandler(OrganizationContactException.class)
    ResponseEntity<ProblemDetail> contactProblem(
            OrganizationContactException exception, HttpServletRequest request) {
        var mapping = switch (exception.reason()) {
            case INVALID_REQUEST -> new ProblemMapping(
                    HttpStatus.BAD_REQUEST,
                    "organization-contact-invalid",
                    "Invalid organization address or contact");
            ...
            case PRECONDITION_REQUIRED -> new ProblemMapping(
                    ...
                    "contact-precondition-required",
                    "Record precondition required");
            case STALE_REVISION -> new ProblemMapping(
                    HttpStatus.PRECONDITION_FAILED,
                    "contact-stale-revision",
                    "Organization address or contact changed");
            case DUPLICATE -> new ProblemMapping(
                    HttpStatus.CONFLICT, "m1.duplicate", "Duplicate organization contact");
            case EFFECTIVE_OVERLAP -> new ProblemMapping(
                    HttpStatus.CONFLICT, "m1.effective.overlap", "Primary record overlap");
            case INVALID_TRANSITION -> new ProblemMapping(
                    HttpStatus.CONFLICT,
                    "m1.lifecycle.invalid_transition",
                    "Invalid record transition");
        };
        var response = response(mapping, exception.getMessage(), request);
        if (exception.reason() == OrganizationContactException.Reason.INVALID_REQUEST
