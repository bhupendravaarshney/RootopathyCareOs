...
public class JdbcFacilityStore implements FacilityStore {
 ...
 """,(r,n)->new FacilitySummary(r.getObject(1,UUID.class),r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6),r.getString(7),r.getLong(8),r.getTimestamp(9).toInstant(),r.getTimestamp(10).toInstant()),c.organizationId(),q,q,q,s,s);return new FacilityDirectory(c.organizationId(),permissions(c).contains(FacilityService.MANAGE),types,rows,Objects.requireNonNull(jdbc.queryForObject("SELECT clock_timestamp()",Timestamp.class)).toInstant());}
