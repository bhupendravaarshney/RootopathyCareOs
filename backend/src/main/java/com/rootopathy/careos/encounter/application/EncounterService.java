...
public final class EncounterService {
    ...
    private static final String PURPOSE = "encounter";
    ...
    private static final Pattern ETAG = Pattern.compile(
            "\\\"m5:(P5-[0-9]{2}):([0-9a-fA-F-]{36}):([0-9]{1,19})\\\"");
    ...
    public EncounterScreen screen(ReadCommand command) {
        Objects.requireNonNull(command, "command");
        var spec = EncounterScreenCatalogue.screen(command.screenId());
        if (command.cursor() != null && !command.cursor().isBlank()) {
            throw invalid("The encounter cursor is invalid or expired; restart from the first page.");
        }
        var limit = command.limit() == null ? 50 : command.limit();
        if (limit < 1 || limit > 100) throw invalid("limit must be between 1 and 100.");
        var query = new EncounterStore.ScreenQuery(
                command.screenId(),
                command.patientId(),
                command.episodeId(),
                command.encounterId(),
    ...
    public IdempotencyOutcome act(ActionCommand command) {
        Objects.requireNonNull(command, "command");
        var spec = EncounterScreenCatalogue.mutation(command.screenId(), command.actionKey());
        ...
        var mutation = new EncounterStore.MutationCommand(
                command.screenId(),
                command.actionKey(),
                command.targetId(),
                command.patientId(),
        ...
        return governedMutations.execute(
                ...
                context -> {
                    ...
                    var response = project(
                            context,
                            EncounterScreenCatalogue.screen(command.screenId()),
                            new EncounterStore.ScreenQuery(
                                    command.screenId(),
                                    result.patientId(),
                                    result.episodeId(),
                                    result.encounterId(),
    ...
    private EncounterScreen project(
            com.rootopathy.careos.tenancy.domain.AuthorizedTenantContext context,
            EncounterScreenCatalogue.ScreenSpec spec,
            EncounterStore.ScreenQuery query) {
        var projection = store.projection(context, query);
        var actions = EncounterScreenCatalogue.projectedActions(spec, store.permissions(context));
        var permittedTargets = actions.stream()
                .filter(EncounterScreen.Action::targetRequired)
                .map(EncounterScreen.Action::key)
                .collect(java.util.stream.Collectors.toUnmodifiableSet());
        var rows = projection.rows().stream()
                .map(row -> new EncounterScreen.Row(
                        row.id(),
                        row.patientId(),
                        row.episodeId(),
                        row.encounterId(),
    ...
    private GovernedMutation governed(
            EncounterStore.MutationResult result,
            String reason,
            EncounterScreen response) {
    ...
    private static void requireFields(
            EncounterScreenCatalogue.ActionSpec spec, Map<String, String> fields) {
        var supported = spec.fields().stream()
                .map(EncounterScreen.Field::key)
                .collect(java.util.stream.Collectors.toUnmodifiableSet());
    ...
    private static Long optionalRevision(String value, String screenId, UUID targetId) {
        ...
        if (!matcher.matches()
            ...
            throw invalid("If-Match does not identify the requested encounter resource.");
        }
        try {
    ...
    private String json(Object value) {
        try {
            ...
            throw new IllegalStateException("Unable to serialize the encounter response.", exception);
        }
    }
