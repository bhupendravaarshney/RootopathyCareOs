...
@Component
public final class AesGcmPatientSensitiveValueCodec implements PatientSensitiveValueCodec {
    private static final byte[] MAGIC = {'C', 'P', 'S', '1'};
    ...
    public AesGcmPatientSensitiveValueCodec(
        ...
        this.encryptionKey =
                new SecretKeySpec(derive(decoded, "careos-patient-sensitive-encryption-v1"), "AES");
        this.digestKey = derive(
                digestKey.getBytes(StandardCharsets.UTF_8),
                "careos-patient-sensitive-digest-v1");
    }
    ...
    public byte[] encrypt(UUID organizationId, String field, String normalizedValue) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Patient sensitive-value encryption is unavailable.", exception);
        }
    }
    ...
    public String digest(UUID organizationId, String field, String normalizedValue) {
        ...
        try {
            ...
            mac.update((byte) 0);
            return HexFormat.of()
                    .formatHex(mac.doFinal(normalizedValue.getBytes(StandardCharsets.UTF_8)));
        } catch (GeneralSecurityException exception) {
            throw new IllegalStateException("Patient sensitive-value keying is unavailable.", exception);
        }
    }

    private static byte[] aad(UUID organizationId, String field) {
        return ("careos-patient-sensitive-v1\0" + organizationId + "\0" + field)
    ...
    private static byte[] derive(byte[] source, String context) {
        try {
            ...
            throw new IllegalStateException("Patient key derivation is unavailable.", exception);
        }
    }

    private static void requireInputs(UUID organizationId, String field, String normalizedValue) {
        if (organizationId == null || field == null || field.isBlank() || normalizedValue == null) {
