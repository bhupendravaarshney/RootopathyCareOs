...
@Component
public final class HmacPatientImpactTokenCodec implements PatientImpactTokenCodec {
    ...
    public HmacPatientImpactTokenCodec(
    ...
    public Decoded decode(String token, Binding binding) {
        try {
            ...
            if (!ENCODER.encodeToString(value).equals(token) || value.length <= 32) {
                throw invalid();
            }
    ...
    private byte[] sign(byte[] payload) {
        try {
            ...
            mac.update("careos-patient-impact\0".getBytes(StandardCharsets.UTF_8));
            return mac.doFinal(payload);
        } catch (GeneralSecurityException exception) {
            throw new IllegalStateException("Patient impact token signing is unavailable.", exception);
        }
    }

    private static IllegalArgumentException invalid() {
        return new IllegalArgumentException(
                "The patient impact preview is invalid, expired, or does not match this action.");
    }
}
