...
public final class AssessmentService {
    ...
    private static final String PURPOSE = "assessment";
    ...
    private static final Pattern ETAG = Pattern.compile(
            "\\\"m6:(COS-[0-9]{2}):([0-9a-fA-F-]{36}):([0-9]{1,19})\\\"");
    ...
    public AssessmentScreen screen(ReadCommand command) {
        Objects.requireNonNull(command, "command");
        var spec = AssessmentScreenCatalogue.screen(command.screenId());
        if (command.cursor() != null && !command.cursor().isBlank()) {
            throw invalid("The assessment cursor is invalid or expired; restart from the first page.");
        }
        var limit = command.limit() == null ? 50 : command.limit();
        if (limit < 1 || limit > 100) throw invalid("limit must be between 1 and 100.");
        var query = new AssessmentStore.ScreenQuery(
                command.screenId(),
                command.patientId(),
                command.encounterId(),
    ...
    public IdempotencyOutcome act(ActionCommand command) {
        Objects.requireNonNull(command, "command");
        var spec = AssessmentScreenCatalogue.mutation(command.screenId(), command.actionKey());
        ...
        var mutation = new AssessmentStore.MutationCommand(
                command.screenId(),
                command.actionKey(),
                command.targetId(),
                command.patientId(),
        ...
        return governedMutations.execute(
                ...
                context -> {
                    ...
                    var response = project(
                            context,
                            AssessmentScreenCatalogue.screen(command.screenId()),
                            new AssessmentStore.ScreenQuery(
                                    command.screenId(),
                                    result.patientId(),
                                    result.encounterId(),
    ...
    private AssessmentScreen project(
            com.rootopathy.careos.tenancy.domain.AuthorizedTenantContext context,
            AssessmentScreenCatalogue.ScreenSpec spec,
            AssessmentStore.ScreenQuery query) {
        var projection = store.projection(context, query);
        var actions = AssessmentScreenCatalogue.projectedActions(spec, store.permissions(context));
        var permittedTargets = actions.stream()
                .filter(AssessmentScreen.Action::targetRequired)
                .map(AssessmentScreen.Action::key)
                .collect(java.util.stream.Collectors.toUnmodifiableSet());
        var rows = projection.rows().stream()
                .map(row -> new AssessmentScreen.Row(
                        row.id(),
                        row.patientId(),
                        row.encounterId(),
    ...
    private GovernedMutation governed(
            AssessmentStore.MutationResult result,
            String reason,
            AssessmentScreen response) {
    ...
    private static void requireFields(
            AssessmentScreenCatalogue.ActionSpec spec, Map<String, String> fields) {
        var supported = spec.fields().stream()
                .map(AssessmentScreen.Field::key)
                .collect(java.util.stream.Collectors.toUnmodifiableSet());
    ...
    private static Long optionalRevision(String value, String screenId, UUID targetId) {
        ...
        if (!matcher.matches()
            ...
            throw invalid("If-Match does not identify the requested assessment resource.");
        }
        try {
    ...
    private String json(Object value) {
        try {
            ...
            throw new IllegalStateException("Unable to serialize the assessment response.", exception);
        }
    }
