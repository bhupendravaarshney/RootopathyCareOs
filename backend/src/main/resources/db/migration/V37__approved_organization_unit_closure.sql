...
BEGIN
 ...
 IF NEW.organization_id IS DISTINCT FROM configured_organization OR configured_actor IS NULL
  OR configured_operation NOT IN ('network.facility.manage','network.structure.lifecycle','configuration.activate')
 ...
 IF TG_OP='UPDATE' AND configured_operation IN ('network.structure.lifecycle','configuration.activate') THEN
  IF NOT ((OLD.status='draft' AND NEW.status='active') OR (OLD.status='active' AND NEW.status='suspended')
   ...
   OR NEW.parent_id IS DISTINCT FROM OLD.parent_id OR NEW.unit_code<>OLD.unit_code OR NEW.unit_type<>OLD.unit_type OR NEW.name<>OLD.name
   OR NEW.effective_from<>OLD.effective_from OR (NEW.status<>'closed' AND NEW.effective_to IS DISTINCT FROM OLD.effective_to) THEN
    RAISE EXCEPTION 'invalid organization unit lifecycle transition' USING ERRCODE='23514';
  END IF;
  IF NEW.status='active' THEN
   ...
   IF NEW.effective_from>clock_timestamp() OR (NEW.effective_to IS NOT NULL AND NEW.effective_to<=clock_timestamp()) THEN
    RAISE EXCEPTION 'organization unit activation requires a current effective range' USING ERRCODE='23514'; END IF;
  END IF;
  IF OLD.status='active' AND NEW.status='suspended' AND EXISTS (
   WITH RECURSIVE descendants(id,status) AS (SELECT id,status FROM organization_units WHERE organization_id=NEW.organization_id AND parent_id=NEW.id UNION ALL SELECT u.id,u.status FROM organization_units u JOIN descendants d ON u.parent_id=d.id WHERE u.organization_id=NEW.organization_id)
   SELECT 1 FROM descendants WHERE status='active') THEN
    RAISE EXCEPTION 'organization unit suspension requires all descendants to be non-active' USING ERRCODE='23514';
  END IF;
  IF NEW.status='closed' THEN
   IF NEW.effective_to IS NULL OR NEW.effective_to<=NEW.effective_from OR abs(extract(epoch FROM (NEW.effective_to-clock_timestamp())))>300 THEN
    RAISE EXCEPTION 'organization unit closure requires a current effective end' USING ERRCODE='23514'; END IF;
   IF EXISTS(WITH RECURSIVE descendants(id,status) AS (SELECT id,status FROM organization_units WHERE organization_id=NEW.organization_id AND parent_id=NEW.id UNION ALL SELECT u.id,u.status FROM organization_units u JOIN descendants d ON u.parent_id=d.id WHERE u.organization_id=NEW.organization_id) SELECT 1 FROM descendants WHERE status<>'closed') THEN
    RAISE EXCEPTION 'organization unit closure requires all descendants to be closed' USING ERRCODE='23514'; END IF;
  END IF;
 END IF;
 IF NEW.parent_id IS NOT NULL THEN
  ...
  IF TG_OP='UPDATE' AND configured_operation IN ('network.structure.lifecycle','configuration.activate') AND NEW.status='active' AND parent_status<>'active' THEN
    RAISE EXCEPTION 'organization unit activation requires an active parent' USING ERRCODE='23514'; END IF;
  WITH RECURSIVE ancestors(id,parent_id,depth) AS (SELECT id,parent_id,1 FROM organization_units WHERE organization_id=NEW.organization_id AND id=NEW.parent_id UNION ALL SELECT p.id,p.parent_id,a.depth+1 FROM organization_units p JOIN ancestors a ON p.id=a.parent_id WHERE p.organization_id=NEW.organization_id AND a.depth<9)
