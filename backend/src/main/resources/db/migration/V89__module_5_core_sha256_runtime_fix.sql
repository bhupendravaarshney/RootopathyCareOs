CREATE FUNCTION careos_m5_sha256(value text)
...
BEGIN
    ...
    IF expected_version IS NULL OR NEW.version_number<>expected_version
       ...
       OR NEW.content_digest<>careos_m5_sha256(NEW.content_text::text)
...
BEGIN
    ...
    IF nullif(current_setting('app.current_operation_key',true),'')<>'encounter.amend'
       ...
       OR NEW.amendment_digest<>careos_m5_sha256(NEW.amendment_text::text)
...
COMMENT ON FUNCTION careos_m5_sha256(text) IS
    'Dependency-free Module 5 SHA-256 helper backed by PostgreSQL core sha256(bytea).';
