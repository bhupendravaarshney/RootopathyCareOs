...
BEGIN
 ...
 IF TG_OP='UPDATE' AND configured_operation='network.structure.lifecycle' THEN
  IF NOT ((OLD.status='draft' AND NEW.status='active') OR (OLD.status='active' AND NEW.status='suspended') OR (OLD.status='suspended' AND NEW.status='active'))
   ...
   OR NEW.effective_from<>OLD.effective_from OR NEW.effective_to IS DISTINCT FROM OLD.effective_to THEN
  ...
  IF OLD.status='active' AND NEW.status='suspended' AND EXISTS (
   WITH RECURSIVE descendants(id,status) AS (
    SELECT id,status FROM organization_units WHERE organization_id=NEW.organization_id AND parent_id=NEW.id
    UNION ALL SELECT u.id,u.status FROM organization_units u JOIN descendants d ON u.parent_id=d.id WHERE u.organization_id=NEW.organization_id)
 ...
 IF NEW.parent_id IS NOT NULL THEN
  ...
  IF TG_OP='UPDATE' AND configured_operation='network.structure.lifecycle' AND NEW.status='active' AND parent_status<>'active' THEN
