...
V14-V18 retain the original reference mechanics; V20 records the approved package; V21-V24 deliver bounded membership/owner/MFA behavior; V25-V29 implement M1-07 through M1-11; V30 establishes the approved M1-12 facility persistence, draft-governance, and evidence boundary; and V31 adds the governed checked facility-draft update event. Invitations, MFA administration, identifier verification, and membership administration use their approved assurance rules and can be enabled in production only with the exact approved registry/package digest; reference service identities remain production-disabled. Canonical registries remain migration-owned and runtime-read-only. M1-05 through M1-12 plus M1-20 implement approved bounded slices, while M1-13 currently includes only its draft-edit prerequisite. Facility lifecycle, facility-scoped membership grants, scheduled automation, configuration invalidation/activation, and final slice acceptance remain unavailable.
